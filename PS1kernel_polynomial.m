...
for j=1:5
    ...
    for h=1:5
        ...
        X4=data2(:,1:end-1);
        Y4=data2(:,end);
        [b,Z_SV,X_SV,Y_SV]=SVM(X3,Y3,c(k),'polynomial',q(j));
        K=kernel(X_SV,X4,q(j),'polynomial');
    ...
    [b2,Z_SV2,X_SV2,Y_SV2]=SVM(X1,Y1,c(num),'polynomial',q(j));
        K1=kernel(X_SV2,X1,q(j),'polynomial');
        y_hat1=sign((Z_SV2.*Y_SV2)'*K1+b2)';
    
    train_err(j) = classification_error(y_hat1,Y1);
    
     K2=kernel(X_SV2,X2,q(j),'polynomial');
...
hold on
plot(q,val_err,'ko-');
title('PS1 Kernel Polynomial Errors');
